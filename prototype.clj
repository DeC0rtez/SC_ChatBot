...
(def alt_food #{"eat", "dinner", "lunch", "breakfast", "food", "dine", "diner", "hunger", "hungry", "snack", "meal", "meals", "drink", "drinking", "drinks", "restaurant", "restaurants"})
(def alt_toilet #{"WC", "restroom", "restrooms", "bathroom", "toilets", "toilet", "WCs", "outhouse", "lavatory"})
(def alt_POI #{"attractions", "sights", "sight", "attraction", "activity", "activities", "tourist attraction", "tourist attractions"})
...
(def alt_dogs #{"Dog", "dog", "doggo", "doggie", "pet", "dogs"})
(def alt_path #{"surface", "path", "pathway", "walkways", "paths", "pathways", "pavement", "pavements", "walkway", "trail", "trails", "footpath", "footpaths"})
(def alt_administration #{"responsible", "administration", "administered", "managed", "managing", "contact", "authority", "manager"})
(def alt_hours #{"time", "hours", "opening hours", "open", "close", "when", "how long", "closing", "opening"})
(def alt_map #{"map", "maps", "mapped", "overview"})


(defn assign_keyword_category 
[keyword]
  (cond
    (contains? alt_food keyword) (def category :food)
    (contains? alt_toilet keyword) (def category :toilet)
    (contains? alt_POI keyword) (def category :POI)
    (contains? alt_bike keyword) (def category :bike_path)
    (contains? alt_skating keyword) (def category :skating)
    (contains? alt_sports keyword) (def category :sports_field)
    (contains? alt_playground keyword) (def category :playground)
    (contains? alt_public_transport keyword) (def category :public_transport)
    (contains? alt_coords keyword) (def category :GPS_coords)
    (contains? alt_parking keyword) (def category :parking)
    (contains? alt_dogs keyword) (def category :dogs)
    (contains? alt_path keyword) (def category :path_type)
    (contains? alt_administration keyword) (def category :administered_by)
    (contains? alt_hours keyword) (def category :opening_hours)
    (contains? alt_map keyword) (def category :map_url)
