...
(def phrases_true {
        ...
        :finish {
            :p1 "Thank you for using our bot!"
            :p2 "Thanks, have a good day!"
            :p3 "Have a good day!"
        }
        })
...
(def synonyms_finish #{"finish", "stop", "end", "exit"}) 
(defn contains_kwd? 
[syn_set str]
...
[keyword]
  (cond
    (contains_kwd? synonyms_finish keyword) :finish
  ...
  (loop [input (read-line)] ;takes user input, stores it in variable
      (do
        ...
        (cond
           ...
           (= category :finish) ((get_response phrases_true :finish) (System/exit 0))
